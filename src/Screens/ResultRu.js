...
const db = openDatabase({name: 'wordhunt.db', createFromLocation: 1});
...
//       getRuWordDetails(id, word).then((res) => {
//         // Обработка примеров (точно как в старом fetchData)
//         const formattedDic = res.dictionary.map((item) => {
//           let newItem = { ...item };
...
//         setState({
//           main: res.main,
//           s1: formattedDic.filter((i) => i.section === 1),
//           s2: formattedDic.filter((i) => i.section === 2),
//           inDic: res.inDictionary,
//           loading: false,
//         });
//       });
//     }
...
//   // --- Вспомогательные рендер-функции (как в оригинале) ---
...
//             renderSectionHeader={({ section }) =>
//               section.title.length > 0 ? (
//                 <Text style={[styles.translationItalic, { marginTop: 35 }]}>
//                   {section.title}
//                 </Text>
//               ) : null
//             }
